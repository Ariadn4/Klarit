...
## 12. 详情页侧抽屉 + 询问 Agent 底部抽屉（增量）

- [x] 12.1 App 布局：主区改 flex 行，看板 flex-1；详情为右侧推挤式侧抽屉（无遮罩），关闭时不占宽；先测试/手验推挤
- [x] 12.2 `RequirementCardDetail` 改抽屉外壳：去蒙层/居中，头部 + 任务内容区（可滚动）+ 底部 CardConsultPanel（绝对定位底部抽屉）
- [x] 12.3 `CardConsultPanel` 改底部抽屉：默认折叠（只露把手栏 + 输入）、发送或点把手展开（覆盖任务内容）、点把手收起；先测试折叠/展开/发送自动展开
- [x] 12.4 清空按钮移入把手栏、做成明确图标按钮 + 内联二次确认；标题「询问 Agent」；先测试
- [x] 12.5 语义令牌、深浅双主题；i18n（展开/收起、把手）
