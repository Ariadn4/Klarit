...
- **可清空该卡会话**：咨询区提供「清空对话」入口——清掉该卡会话的**消息历史**并**断开原生续接**（清 sessionId），保留会话本身（id=cardId）与所选 agent/模型；清空须**二次确认**（不用原生弹窗）。清空后回到空态、可重新开聊。
- **消息底部操作（复制/编辑/重试）**：对齐全局对话——每条消息底部提供操作按钮，顺序与全局一致（**复制 → 编辑 → 重试**）：所有消息都有「复制」（复制整条文字，含 agent 回复 + 提案/干预可读描述）；**仅最新一条用户消息**另有「编辑」（回填输入 + 移除该轮）与「重试」（丢弃该轮 agent 回复、按本会话选型重跑该条用户意图）。
- **可打断思考**：咨询轮进行中（agent 思考/产出时），提供「停止」入口打断——中止该卡正在跑的咨询 agent，运行回到可输入态（该轮不再追加内容或只留已产出的部分）。
