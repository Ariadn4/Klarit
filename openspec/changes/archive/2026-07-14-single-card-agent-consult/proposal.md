...
- **详情页改为侧抽屉 + 底部 Agent 抽屉**：需求卡详情从居中弹窗改为**从右侧推挤看板的侧抽屉**（无遮罩、看板让位、顶部 ✕ 关闭）；「询问 Agent」改为抽屉**底部的可开合抽屉**——**默认折叠**、只露把手栏（「询问 Agent」+ 清空按钮 + 展开/收起）与输入框；**发送问题或拉起把手即展开**（向上滑开、**覆盖**任务内容区显示对话），点把手可收回折叠；「清空对话」为把手栏上的**明确按钮**（图标 + 文案，点后内联二次确认）。
