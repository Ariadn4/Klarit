...
分解所用的 prompt SHALL 按确定顺序解析：**高级覆盖 skill（当前项目激活工作流的「新建需求」prompt，或全局手写/导入的覆盖 skill，若声明）→ 由类型注册表自动生成的分解 skill（兜底，始终可得）**。即：声明了覆盖 skill 时用覆盖 skill；否则用自动生成的分解 skill。解析 MUST 始终得到一份非空生效 prompt（自动生成的分解 skill 是兜底来源，只要注册表至少有默认类型即非空，保证无工作流/无覆盖时分解仍可进行）。
...
- **THEN** 系统以由类型注册表自动生成的分解 skill 作为生效 prompt，分解仍可进行

### Requirement: 分解 skill 由类型注册表自动生成
