...
> **已于 2026-08-31 补测**（Klarit 自身仓 29k 文件，直起真实 claude CLI）：**A 证伪**——挂仓根与不挂的 CPU 一样
> （5.23 vs 5.57 秒，噪声内），`--add-dir` 不做索引。**B 的成因成立但提速测不出来**——挂得少确实让 agent 少翻
> （轮次 −42%、工具调用 −47%），可省下的轮次被「每轮更大」吃回去：墙钟 +15%、成本 +16%、CPU −5%(噪声内)。
> ⚠️ 首轮量出的「CPU −46%」是**工装错误**（只喂裸 intent 428 字节，漏了生产 prompt 里内联的 workflow schema，
> 逼着挂仓根那组去读 `src/shared/workflow.ts`）；忠实 prompt（21.7 KB）复测后该数字作废。
> 「CPU 上万秒」未能复现。完整数字与偏差说明见 design.md「先量，别猜」。

- [x] 0.1 在一个大仓上测 **A（`--add-dir` 本身贵）**：挂仓根、只问一句废话，记 CPU 累计与耗时
- [x] 0.2 测 **B（agent 自己翻贵）**：挂一个小目录、问同样的 author 问题，记 CPU 累计与耗时
- [x] 0.3 把结论与数字写回 `design.md`；结论＝**提速收益测不出来**，触发了本条「若收益有限先回来重议」的闸门。
      重议结果：**不推翻重做**（轮次/工具调用的收敛是真的、也没测出质量损失），但**不得再拿「提速」为它辩护**；
      design.md 里「快一个数量级」「换 CPU」的说法已就地更正
...
- [ ] 6.3 dogfood **两项都要测**（(a) 已结案：无提速；(b) 仍未定论）：
      (a) **提速** —— ✅ 已测，结论是**没有**：墙钟 +15%、成本 +16%、CPU −5%(噪声内)。注意是**直起 CLI** 量的，
          未走 Klarit 应用内完整链路，应用层开销未计入
      (b) **是否漏** —— ⚠️ 测了但 n=2 判不出：改前习惯覆盖 9/11 与 6/11，改后 6/11 与 7/11，改前自身波动即盖过
          组间差异。要判得在真项目上多跑几轮比产出质量，别再量 CPU
