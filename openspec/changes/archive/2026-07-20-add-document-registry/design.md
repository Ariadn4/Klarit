...
- 读文档内容样本，为每条与项目整体起草「习惯 prompt」，确认保存（=整表审批）后方生效。
...
- **分析费 token / 慢**：agent 一次调用读清单+样本，大仓可达一两分钟。缓解——样本取样限流（每夹 2 个、全局 30 个、单样本截断）；统一推出 + 加载指示保证等待可感知；无 agent / 失败回落启发式只出分类表，配好后「重新扫描」补。
