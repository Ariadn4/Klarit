...
## Purpose
定义工作流的数据模型与「包目录」持久化：阶段（看板列）+ 有序节点列表，节点的执行者（agent/engine/command/subworkflow）、agent 驱动指令的两种形态、产出/可写范围/门，以及以 `userData/workflows/<id>/` 包目录（`workflow.yaml` + skill 文件）的读写、校验与内置默认种子。
## Requirements
