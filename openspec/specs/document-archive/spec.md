...
## Purpose
定义「归档」引擎操作 `archive-docs` 的语义：读某成员仓的文档登记表（来自 `document-registry` 的 `document-store`）+ 审批过的 habitPrompt/项目公约，按 `ManagedDoc.kind` 路由归档（动态就地更新 / 快照按习惯追加冻结），照习惯决定写不写与怎么写；运行时支持子 agent 时并行、否则串行退化；多仓各归各仓；缺表/无 agent/空表各有清楚兜底。归档产生文档写入并提交。
