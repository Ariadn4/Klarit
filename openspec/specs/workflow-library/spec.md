...
## Purpose
工作流库的管理操作——列出、新建、克隆、删除、导入/导出（均以工作流「包」为单位），以及工作流包内 skill 文件的新建/导入管理，并经 IPC 暴露给渲染层。
...
系统 SHALL 提供按相对包路径读取 skill 文件内容的能力（供编辑界面查看），越界路径（绝对/含 `..`）MUST 被拒。当某 `file` 引用被移除或改为 `inline` 时，系统 MAY 清理不再被引用的包内 skill 文件；任何情况下 MUST 不留下指向包外的引用。
...
#### Scenario: 按相对包路径读取 skill 内容
- **WHEN** 请求读取某工作流包内一份 skill 文件的内容
- **THEN** 系统返回该文件内容；若路径越界（绝对路径或含 `..`）或文件不存在则返回空
