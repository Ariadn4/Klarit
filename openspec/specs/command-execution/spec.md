...
「永不抛」MUST 覆盖**取消/杀进程树这条路径本身**:杀进程树所依赖的手段可能失败(拉不起杀进程的外部命令、目标进程已退出、权限不足),这类失败 MUST 被吞掉——既不同步抛出,也不得以未捕获的异步错误事件冒泡到进程级。杀进程树是**尽力而为**的:它失败不改变运行器的对外契约,运行器仍以 `killed` 为真正常 resolve。此约束同样适用于**延后执行**的兜底终止(如宽限期后的强杀),因为它在事件循环的后续轮次里跑,抛出即为未捕获异常。
...
#### Scenario: 杀进程树的手段本身失败也不崩进程
- **WHEN** 取消一条正在跑的命令,而杀进程树所依赖的手段失败(如拉不起杀进程的外部命令、目标进程已退出、权限不足)
- **THEN** 该失败被吞掉,不抛出、也不产生未捕获的进程级错误,运行器仍以 `killed` 为真正常 resolve

#### Scenario: 延后的兜底强杀失败也不崩进程
- **WHEN** 取消后进入宽限期,宽限期结束时的兜底强杀在事件循环的后续轮次里执行并失败
- **THEN** 该失败被吞掉,不产生未捕获异常,不影响已 resolve 的运行结果
