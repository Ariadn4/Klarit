# card-ops-review-apply Specification

## Purpose
把编排核产出的卡操作提案以审阅形态呈现，供用户逐条勾选、破坏性操作二次确认，人确认后经 cardStore 的 `applyOps` 接缝落库应用；非法 op 视为系统问题对用户剔除并记录；新项目提议确认后创建项目并种入需求卡；外部 AI 产出的 ops 经同一接缝进人审与应用。
