# git-write-operations Specification

## Purpose
定义 git 写侧：异步 git 运行器、写侧四件套（建/删分支、加/删 worktree）、合并（冲突即 abort 回干净态）、推送（非快进/无远端结构化失败）、junction 链接与防御性解链（绝不递归进 reparse point），均为可独立测试的纯函数 + 结构化结果。
## Requirements
