...
**抛决策 MUST 先落盘、再发事件**:引擎把待决策写进断点后,MUST **先持久化断点**,才发 `decision` 进度事件。消费方(如决策收件箱投影)是**按 runId 回读持久化断点**来认这条决策的;若先发事件后落盘,消费方在收到事件的那一刻从盘上读到的仍是旧断点、看不见 `pendingDecision`,于是把这条决策当作「已消失」而丢弃——表现为条目最终仍能由全量重建补上(计数正确),但**只为新增条目触发的副作用(桌面通知)永久丢失**。同一约定已适用于门重试事件(先存计数再发 `gate-retry`)。

#### Scenario: 决策事件发出时断点已可回读
- **WHEN** 引擎抛出一条待决策并发出 `decision` 事件
- **THEN** 事件处理方在该刻按 runId 回读持久化断点,即可读到这条 `pendingDecision`(而非旧断点)
