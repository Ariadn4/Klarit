...
${content.slice(0, SAMPLE_CAP)}`
  ...
  return [
    ...
    '每条起草一段「习惯 prompt」：只写从样本能读出的**格式类事实**（模板结构、字段、命名规则、时态语气），不要猜写作频率或意图。',
    '另起草一段项目级「文档公约」——跨文件的通用写作约定。',
    '习惯 prompt 与文档公约都**只写正向要求与示例**（要怎么写、举正确写法的例子），不用反例或禁止式表述。',
    '',
    '# 候选文档清单（相对仓根）',
    ...candidates,
    '',
    ...
    '严格只输出一个 JSON 对象：',
    '{"convention": "<项目级公约>", "entries": [{"location": "<路径>", "kind": "dynamic|snapshot", "habitPrompt": "<习惯 prompt>"}]}，',
